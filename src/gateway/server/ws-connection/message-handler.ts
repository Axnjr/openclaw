  ...
  socket.on("message", async (data) => {
    ...
    try {
      ...
      if (!client) {
        ...
        const allowNativeSharedAuthScopeBypass =
          isNativeAppClient &&
          configSnapshot.gateway?.controlUi?.dangerouslyDisableDeviceAuth === true &&
          sharedAuthOk;
        const allowUnpairedScopeBypass = allowControlUiBypass || allowNativeSharedAuthScopeBypass;
        ...
        if (!device) {
          if (scopes.length > 0 && !allowUnpairedScopeBypass) {
            scopes = [];
            connectParams.scopes = scopes;
          }
